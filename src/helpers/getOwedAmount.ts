  ...
  for (const concepto of conceptos) {
    ...
    if (participante) {
      ...
      if (typeof participante === "object" && !participante.pagado) {
        amountOwed +=
          (concepto.precio * concepto.unidades) / concepto.participantes.length;
      }
    }
    if ((concepto.pagador as any)._id === user._id && !isContable) {
      const debtUsers = (concepto.participantes as any[]).reduce(
        (acc, p) => (p.pagado ? acc : ++acc),
        0
      );
      // If the user is the pagador for this concepto, add the total cost to the amount owed
      amountOwed -=
        (((concepto.participantes as any[]).find(
          (p: any) => p.usuario._id === user._id
        )?.pagado
          ? 0
          : 1) *

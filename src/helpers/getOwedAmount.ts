...
export function getAmountOwed(conceptos: Concepto[], user: User, isContable: Boolean): number {
    ...
    for (const concepto of conceptos) {
      ...
      if ((concepto.pagador as any)._id === user._id && !isContable) {
        ...
        amountOwed -= ((concepto.participantes as any[]).find((p: any) => p.usuario._id === user._id).pagado ? 0 : 1)*(concepto.precio * concepto.unidades) * debtUsers / concepto.participantes.length;
      }
    }
